...
|---|---|---|---|
| 0.1 | **Git branch** | Create feature branch `feature/vue-inertia-migration` | — |
| 0.2 | **Verify .env** | Confirm local database connection, run `php artisan migrate` | `.env` |
| 0.3 | **Verify .env.prod** | Document production config (reference only, do not modify) | `.env.prod` |
| 0.4 | **Full backup** | `git commit` current state, tag as `pre-migration` | — |
| 0.5 | **Smoke test** | Verify current app runs: `php artisan serve` + login + navigate pages | — |
| 0.6 | **PHP version check** | Confirm local PHP >= 8.2 (required for Laravel 11) | — |

**Checkpoint:** All tasks must pass before Phase 1.
